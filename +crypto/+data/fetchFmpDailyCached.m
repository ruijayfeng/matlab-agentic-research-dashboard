function candles = fetchFmpDailyCached(symbol, apiKey, maxAgeHours)
%FETCHFMPDAILYCACHED Read FMP daily candles with local caching.
    arguments
        symbol (1,1) string
        apiKey (1,1) string
    ...
    try
        candles = crypto.data.fetchFmpDaily(symbol, apiKey);
...
function cacheFile = localCacheFile(symbol)
    ...
    cacheFile = fullfile(pwd, "cache", "fmp", fileName);
end
